...
fn parse_address(value: &str) -> [u8; 20] {
    ...
    assert_eq!(value.len(), 40, "address must be 20 bytes");
...
struct ProofInput {
    ...
    logical_taker: [u8; 20],
    tfund: [u8; 20],
    settlement_token: [u8; 20],
    kyc_root: [u8; 32],
    current_timestamp: u64,
    fill_making_amount: [u8; 32],
    order: Order,
}
...
fn sample_input(router: [u8; 20], tfund: [u8; 20], settlement_token: [u8; 20]) -> ProofInput {
    let maker_key = SigningKey::from_bytes((&[1u8; 32]).into()).unwrap();
    let taker_key = SigningKey::from_bytes((&[2u8; 32]).into()).unwrap();
    let maker = address_from_key(&maker_key);
    let logical_taker = address_from_key(&taker_key);
    let kyc_root = keccak256(&maker);
    let mut input = ProofInput {
        ...
        fill_making_amount: amount(1_000_000),
        order: Order {
            salt: amount(1),
            maker,
            receiver: maker,
            maker_asset: tfund,
            taker_asset: settlement_token,
    ...
    input.order.signature = sign_order(&input.order, &maker_key, &domain_separator);
    input
}
...
fn load_input() -> ProofInput {
    match env::var("BATCH_INPUT_PATH") {
        ...
        Err(_) => sample_input(
            parse_address(
                &env::var("ONE_INCH_ROUTER_ADDRESS").expect("ONE_INCH_ROUTER_ADDRESS is required"),
            ),
            parse_address(&env::var("TFUND_ADDRESS").expect("TFUND_ADDRESS is required")),
            parse_address(
                &env::var("SETTLEMENT_TOKEN_ADDRESS")
                    .expect("SETTLEMENT_TOKEN_ADDRESS is required"),
            ),
        ),
    }
}
...
async fn main() {
    ...
    assert_eq!(public_values.len(), 13 * 32, "unexpected ABI public value length");
    let vkey = proving_key.verifying_key().bytes32_raw();
    let output_dir = PathBuf::from(
        env::var("SP1_OUTPUT_DIR").unwrap_or_else(|_| "proofs/rwa-dex-order".to_owned()),
    );
